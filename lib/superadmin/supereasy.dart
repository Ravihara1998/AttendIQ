...
class Supereasy extends StatefulWidget {
  const Supereasy({super.key});

  @override
  State<Supereasy> createState() => _SupereasyState();
}

class _SupereasyState extends State<Supereasy> {
  ...
  Future<void> _loadCompanies() async {
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          _allCompanies = companies.toList()..sort();
          _selectedCompany = _allCompanies.isNotEmpty ? _allCompanies[0] : null;
  ...
  Future<void> _sendEmail() async {
    ...
    showDialog(
      ...
      builder: (context) => EmailDialog(
        ...
        companyName: _selectedCompany ?? 'Unknown Company',
        ...
        if (mounted) {
          ...
          for (int i = 0; i < checkIns.length; i++) {
            ...
            if (_selectedCompany != null &&
              ...
              print(
                'Filtered out: empId=${checkIns[i].empId}, company=$company (not matching $_selectedCompany)',
              );
            }
          }
  ...
  Future<void> _downloadPdf() async {
    ...
    try {
      await PdfGenerator.generateAndDownloadPdf(
        ...
        employeeTotalHours: _employeeTotalHours, // ✅ Pass the per-employee map
      );
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                if (!isMobile)
                  Text(
                    '${_filteredCheckIns.length} records • ${_selectedCompany ?? "All Companies"}',
  ...
  Widget _buildCompanyDropdown(bool isMobile) {
    ...
    if (_allCompanies.isEmpty) {
      return Container(
        ...
        child: Text(
          'No companies found',
          style: EasyStyle.bodyMedium.copyWith(color: EasyStyle.textSecondary),
        ),
      );
    }
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          _buildHeaderCell('Total Hours', 100),
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          _buildTotalHoursCell(totalHours, 110),
